...
function make_mn_data(
   ...
   for subnet in eachrow(subnetworks)
      ...
      mn_data["nw"]["$(subnet.index)"] = networks[file_name]
      mn_data["nw"]["$(subnet.index)"]["variable_f"] = typeof(subnet.variable_f)==Bool ? subnet.variable_f : (lowercase(subnet.variable_f) == "true")
      mn_data["nw"]["$(subnet.index)"]["f_base"] = subnet.f_base
      if mn_data["nw"]["$(subnet.index)"]["variable_f"]
         mn_data["nw"]["$(subnet.index)"]["f_min"] = subnet.f_min
         mn_data["nw"]["$(subnet.index)"]["f_max"] = subnet.f_max
      end

      # REMOVING BUSES OF TYPE 4 (ISOLATED)
      # mn_data["nw"]["$(subnet.index)"]["bus"] = filter(p->(last(p)["bus_type"]!=4), mn_data["nw"]["$(subnet.index)"]["bus"])
      # Disable loads at type 4 buses
      for (idx,load) in mn_data["nw"]["$(subnet.index)"]["load"]
         # println("Bus $(load["load_bus"])")
         # println(mn_data["nw"]["$(subnet.index)"]["bus"]["$(load["load_bus"])"])
         if mn_data["nw"]["$(subnet.index)"]["bus"]["$(load["load_bus"])"]["bus_type"]==4 && load["status"] == 1
            println("Bus $(load["load_bus"]) is type 4 (isolated) and has a load. The load will be disabled.")
            load["status"] = 0
         end
      end
      for (idx,gen) in mn_data["nw"]["$(subnet.index)"]["gen"]
         if mn_data["nw"]["$(subnet.index)"]["bus"]["$(gen["gen_bus"])"]["bus_type"]==4 && gen["gen_status"] == 1
            println("Bus $(gen["gen_bus"]) is type 4 (isolated) and has a generator. The generator will be disabled.")
            gen["gen_status"] = 0
         end
      end
      for (idx,shunt) in mn_data["nw"]["$(subnet.index)"]["shunt"]
         if mn_data["nw"]["$(subnet.index)"]["bus"]["$(shunt["shunt_bus"])"]["bus_type"]==4 && shunt["status"] == 1
   ...
   for interface in eachrow(interfaces)
      ...
      if all(indexin(["R","X","G","B"], names(interfaces)) .!= nothing) && !(any(x -> ismissing(x), interface[["R","X","G","B"]])) && !(interface.R == 0 && interface.X == 0)
         ...
         while new_bus_idx in parse.(Int,keys(mn_data["nw"]["$(interface.subnet_index)"]["bus"]))
            busmult=busmult+1
            new_bus_idx = Int(interface.bus + 900*10^(busmult+floor(log10(interface.bus))))
         end
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"] = Dict{String,Any}()
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["bus_i"] = new_bus_idx
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["index"] = new_bus_idx
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["bus_type"] = 1
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["vmax"] = interface.vmax
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["vmin"] = 0.8
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["zone"] = copy(mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(interface.bus)"]["zone"])
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["area"] = copy(mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(interface.bus)"]["area"])
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["va"] = copy(mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(interface.bus)"]["va"])
         mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["vm"] = copy(mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(interface.bus)"]["vm"])
         if "base_kv" in names(interfaces)
            mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["base_kv"] = interface.base_kv
         else
            mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["base_kv"] = copy(mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(interface.bus)"]["base_kv"])
         end
         # Include current limit in bus data
         # mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["converter_imax"] = Dict{Int64,Float64}()
         # mn_data["nw"]["$(interface.subnet_index)"]["bus"]["$(new_bus_idx)"]["converter_imax"][interface.index] = interface.imax

         # Now define a branch at the terminal
         new_branch_idx = maximum(parse.(Int,keys(mn_data["nw"]["$(interface.subnet_index)"]["branch"]))) + 1
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"] = Dict{String,Any}()
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["index"] = new_branch_idx
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["f_bus"] = new_bus_idx
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["t_bus"] = interface.bus
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["angmax"] = pi/2
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["angmin"] = -pi/2
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["br_r"] = interface.R
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["br_x"] = interface.X
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["g_to"] = interface.G/2
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["g_fr"] = interface.G/2
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["b_to"] = interface.B/2
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["b_fr"] = interface.B/2
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["g_to"] = interface.G/2
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["g_fr"] = interface.G/2
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["rate_a"] = interface.smax
         mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["br_status"] = 1
         if ("transformer" in names(interfaces)) && !(ismissing(interface.transformer))
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["transformer"] = interface.transformer
         else
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["transformer"] = false
         end
         if ("tap" in names(interfaces)) && !(ismissing(interface.tap))
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["tap"] = interface.tap
         else
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["tap"] = 1
         end
         if ("shift" in names(interfaces)) && !(ismissing(interface.shift))
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["shift"] = interface.shift
         else
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["shift"] = 0
         end
         if ("rate_b" in names(interfaces)) && !(ismissing(interface.rate_b))
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["rate_b"] = interface.rate_b
         else
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["rate_b"] = interface.smax
         end
         if ("rate_c" in names(interfaces)) && !(ismissing(interface.rate_c))
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["rate_c"] = interface.rate_c
         else
            mn_data["nw"]["$(interface.subnet_index)"]["branch"]["$(new_branch_idx)"]["rate_c"] = interface.smax
         end
      ...
      if !("converter_index" in keys(mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]))
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_index"] = Array{Tuple,1}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_imax"] = Dict{Int64,Float64}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_vmax"] = Dict{Int64,Float64}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_c1"] = Dict{Int64,Float64}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_c2"] = Dict{Int64,Float64}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_c3"] = Dict{Int64,Float64}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_sw1"] = Dict{Int64,Float64}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_sw2"] = Dict{Int64,Float64}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_sw3"] = Dict{Int64,Float64}()
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_M"] = Dict{Int64,Float64}()
      end
      ...
      if !(interface.index in mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_index"])
         push!(mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_index"], (interface.index,interface_bus[1],interface_bus[2]))
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_imax"][interface.index] = converter_params[:imax]
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_vmax"][interface.index] = converter_params[:vmax]
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_c1"][interface.index]   = converter_params[:c1]
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_c2"][interface.index]   = converter_params[:c2]
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_c3"][interface.index]   = converter_params[:c3]
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_sw1"][interface.index]  = converter_params[:sw1]
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_sw2"][interface.index]  = converter_params[:sw2]
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_sw3"][interface.index]  = converter_params[:sw3]
         mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_M"][interface.index]    = converter_params[:M]
      end
      # println("converter_imax: $(mn_data["nw"]["$(interface_bus[1])"]["bus"]["$(interface_bus[2])"]["converter_imax"])")
   ...
   PowerModels.standardize_cost_terms!(mn_data, order=2)
   return mn_data
end
